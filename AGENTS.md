# Правила рабочей области `C:\MAMP\htdocs`

## 1. Тематические правила

Для учебных задач, связанных с Word/PDF, электротехническими расчётами, MathType или Mathcad, перед началом работы полностью прочитай:

`rules/word-pdf-academic.md`

Эти правила применяются только к соответствующим учебным задачам и не распространяются на обычные программные проекты.
...
Используй стандарты commit messages проекта или `docs/git-flow.md`.
...
`C:\MAMP\htdocs\ai-dev-team-codex-kit`
...
Если задача относится к существующему проекту, сначала проверь наличие:

`C:\MAMP\htdocs\<project>\`

Если preset существует, сначала прочитай только его основной:
...
`ai-dev-team-codex-kit/docs/TEAM_ARCHITECTURE.md`

`ai-dev-team-codex-kit/docs/TEAM_COMMANDS.md`
...
Предпочтительное расположение — в `<repo-root>/docs/`, если структура проекта или локальный `AGENTS.md` не задают другое место.
...
Файлы внутри `ai-dev-team-codex-kit` могут использоваться как библиотека инструкций и шаблонов.
