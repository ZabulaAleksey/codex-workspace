# Правила `~/codex-workspace` и проектов в `~/projects`

## 1. Расположение рабочей области

- AI-инфраструктура и её общие документы находятся в `~/codex-workspace`.
- Шаблоны проектов находятся в `~/codex-workspace/projects/<preset>`.
- Рабочие репозитории находятся только в `~/projects/<project>`.
...
`~/projects/<project>/docs/git-flow.md`
...
`~/codex-workspace/projects/<preset>`

и сначала прочитай только `~/codex-workspace/projects/<preset>/AGENTS.md`.
...
Предпочтительное расположение — в `~/projects/<project>/docs/`, если структура проекта или локальный `AGENTS.md` не задают другое место.
