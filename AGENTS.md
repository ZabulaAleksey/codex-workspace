...
Перед началом задачи применяй правила из `rules/model-routing.md`.

Для небольших и средних задач предпочитай GPT-5.3-Codex-Spark. Основную Codex-модель используй для крупных, архитектурно сложных, long-horizon или существенно неоднозначных задач.

Выбор Spark не ослабляет требования SPEC, DoD, тестирования, review, безопасности и Git workflow.


<!-- AI-DEV-TEAM-TEST-CONTRACT-POLICY -->
## Политика тест-контрактов

- Принятый тест, fixture, golden-снапшот или E2E-сценарий является контрактом поведения. В обычной реализации этот артефакт запускается только для верификации.
- Принятые тесты, fixtures и e2e-сценарии не модифицируются, не удаляются и не пропускаются без явного согласования.
- Невыполненный принятый тест означает дефект в рабочем коде и исправляется именно изменениями в production-коде.
- Изменение уже принятого теста разрешено только по прямому запросу владельца или в отдельной задаче изменения требований.
- Новые тесты могут добавляться вместе с новой функциональностью перед первой приёмкой, но это не даёт права переписывать уже принятые тесты.
